...
if __name__ == "__main__":
    ...
    remaining_feature_names = [all_feature_names[i] for i in range(len(all_feature_names)) if i not in features_eliminate]
    ...
    auc = validation_test(test_predictions, X_test, y_test, model, model_name=f"Modello {ModelName.LOGISTIC_REGRESSION_GD.value}")

    test_sk_predictions = sk_model.predict(X_test)
    sk_auc = validation_test(test_sk_predictions, X_test, y_test, sk_model, model_name=f"Modello {ModelName.SCIKIT_LEARN.value}")
    ...
    if plotting:
        ...
        plot_results(X_test, y_test, model, sk_model, test_predictions, test_sk_predictions, auc, sk_auc, ModelName)
