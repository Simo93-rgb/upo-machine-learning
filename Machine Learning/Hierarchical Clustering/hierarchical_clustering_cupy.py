...
try:
    # Verifica se una GPU è effettivamente disponibile
    if cp.cuda.is_available():
        xp = cp
        HAS_GPU = True
        print("GPU rilevata. Utilizzo CuPy.")
    else:
        xp = np
        HAS_GPU = False
        print("Nessuna GPU rilevata. Utilizzo NumPy.")
except ImportError:
    xp = np
    HAS_GPU = False
    print("CuPy non installato. Utilizzo NumPy.")
...
class HierarchicalClustering:
    def __init__(self, X, linkage: str = 'single'):
        ...
        self.distance_matrix = {str(i): {str(j): xp.array(initial_distances[i, j])
    ...
    def _merge_clusters(self, cluster1_idx: str, cluster2_idx: str, distance: float) -> None:
        ...
        for other in list(self.clusters.keys()):
            ...
            if self.linkage == 'single':
                new_dist = xp.minimum(self.distance_matrix[cluster1_idx][other],
                                      self.distance_matrix[cluster2_idx][other])
            elif self.linkage == 'complete':
                new_dist = xp.maximum(self.distance_matrix[cluster1_idx][other],
                                      self.distance_matrix[cluster2_idx][other])
